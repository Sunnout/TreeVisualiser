 ...
 */
public class FREmaLayout extends AbstractLayout<PlumtreeVertex, PlumtreeEdge> implements IterativeContext {

    protected LoadingCache<PlumtreeVertex, FRVertexData> frVertexData =
            CacheBuilder.newBuilder().build(new CacheLoader<PlumtreeVertex, FRVertexData>() {
                public FRVertexData load(PlumtreeVertex vertex) {
    ...
    public FREmaLayout(Graph<PlumtreeVertex, PlumtreeEdge> g) {
        super(g);
    }

    public FREmaLayout(Graph<PlumtreeVertex, PlumtreeEdge> g, Dimension d) {
        super(g, new RandomLocationTransformer<PlumtreeVertex>(d), d);
        initialize();
    ...
    public void setSize(Dimension size) {
        if (initialized == false) {
            setInitializer(new RandomLocationTransformer<PlumtreeVertex>(size));
        }
        super.setSize(size);
    ...
    private void doInit() {
        Graph<PlumtreeVertex, PlumtreeEdge> graph = getGraph();
        Dimension d = getSize();
    ...
    public synchronized void step() {
        ...
        while (true) {
            ...
            try {
                for (PlumtreeVertex v1 : getGraph().getVertices()) {
                    calcRepulsion(v1);
                }
                break;
        ...
        while (true) {
            try {
                for (PlumtreeEdge e : getGraph().getEdges()) {
                    if (e.getType() == PlumtreeEdge.Type.EAGER)
                        calcAttraction(e);
                }
                break;
            } catch (ConcurrentModificationException cme) {
        ...
        while (true) {
            try {
                for (PlumtreeVertex v : getGraph().getVertices()) {
    ...
    protected synchronized void calcPositions(PlumtreeVertex v) {
    ...
    protected void calcAttraction(PlumtreeEdge e) {
        Pair<PlumtreeVertex> endpoints = getGraph().getEndpoints(e);
        PlumtreeVertex v1 = endpoints.getFirst();
        PlumtreeVertex v2 = endpoints.getSecond();
    ...
    protected void calcRepulsion(PlumtreeVertex v1) {
        FRVertexData fvd1 = getFRData(v1);
        if (fvd1 == null)
            return;
        ...
        try {
            for (PlumtreeVertex v2 : getGraph().getVertices()) {
...
//                if (isLocked(v2)) continue;
    ...
    protected FRVertexData getFRData(PlumtreeVertex v) {
